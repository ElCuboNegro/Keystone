#!/usr/bin/env python3
"""
keystone_gui.py -- Keystone Vault Manager GUI
...
try:
    ...
    from keystone_nfc.exceptions import NoReaderError
    from keystone_nfc.registry import VaultEntry, VaultRegistry
...
class _AddVaultDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Misc) -> None:
        ...
        self.resizable(False, False)
        self.grab_set()
        self.result = None  # (name: str, vault: Path, workdir: Path|None)

        grid_opts = {'sticky': 'w', 'padx': 12, 'pady': 4}

        ttk.Label(self, text='Vault name:').grid(row=0, column=0, **grid_opts)
        self._name = ttk.Entry(self, width=32)
        self._name.grid(row=0, column=1, padx=(0, 4), pady=4)

        ttk.Label(self, text='Vault folder:').grid(row=1, column=0, **grid_opts)
        ...
        ttk.Label(self, text='Workdir (optional):').grid(row=2, column=0, **grid_opts)
...
class App:
    """Keystone Vault Manager — main application class."""
    ...
    def _build_window(self) -> None:
        self.root = tk.Tk()
        self.root.title('Keystone Vault Manager')
    ...
    def _make_vault_row(self, entry: VaultEntry) -> None:
        ...
        if is_open:
            ttk.Button(row, text='Open Folder',
                       command=lambda e=entry: self._open_folder(e)).pack(side='left', padx=2)
            ttk.Button(row, text='Lock',
                       command=lambda e=entry: self._request_lock(e)).pack(side='left', padx=2)
        elif status == 'locked':
            if self._card:
                ttk.Button(row, text='Unlock',
                           command=lambda e=entry: self._ask_password_and_unlock(e)).pack(side='left', padx=2)
            else:
                ttk.Label(row, text='(insert card to unlock)',
                          foreground='#888', font=('', 8)).pack(side='left', padx=6)

        ttk.Button(row, text='Remove',
                   command=lambda e=entry: self._remove_vault(e)).pack(side='right')
    ...
    def _add_vault(self) -> None:
        ...
        if not dlg.result:
            return
        name, vault_path, workdir_path = dlg.result
    ...
    def _ask_password_and_unlock(self, entry: VaultEntry) -> None:
        ...
        dlg = _PasswordDialog(self.root, entry.name, self._card.uid_hex)
    ...
    def _dispatch(self, event: tuple) -> None:
        ...
        if kind == 'card_inserted':
            self._card = event[1]
            self._set_status(f'Card: {self._card.uid_hex}')
            if self._tray:
                self._tray.icon = _make_tray_image(locked=False)
            self._show_window()
            ...
            if self._tray:
                self._tray.icon = _make_tray_image(locked=True)
            self._lock_all()
    ...
    def _start_card_monitor(self) -> None:
        try:
            self._monitor = KeystoneReader()

            @self._monitor.on_card_inserted
            def _on_insert(card: CardInfo):
                _Q.put(('card_inserted', card))

            @self._monitor.on_card_removed
            def _on_remove():
                _Q.put(('card_removed',))
