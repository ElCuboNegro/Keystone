...
## Activated when: retro-engineer requests BDD output, or user invokes /bdd-write
...
**CRITICAL MANDATE: Human-Readable Business Logic**
All generated BDD scenarios and logic specifications MUST be written in natural, business-level language (understandable by non-technical stakeholders). You MUST completely and exhaustively describe the business process, leaving no rule or condition ambiguous or truncated. Do not just list technical SQL or code snippets; translate every filter, calculation, and transformation into its full business meaning.
